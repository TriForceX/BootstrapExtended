			...
			case 'html':
				...
				if(strpos($input, ' style=') !== false) {
					$input = preg_replace_callback('#<([^<]+?)\s+style=([\'"])(.*?)\2(?=[\/\s>])#s', function($matches) {
						return '<' . $matches[1] . ' style=' . $matches[2] . self::minify_code('css', $matches[3]) . $matches[2];
					}, $input);
				}
				if(strpos($input, '</style>') !== false) {
					$input = preg_replace_callback('#<style(.*?)>(.*?)</style>#is', function($matches) {
						return '<style' . $matches[1] .'>'. self::minify_code('css', $matches[2]) . '</style>';
					}, $input);
				}
				if(strpos($input, '</script>') !== false) {
					$input = preg_replace_callback('#<script(.*?)>(.*?)</script>#is', function($matches) {
						return '<script' . $matches[1] .'>'. self::minify_code('js', $matches[2]) . '</script>';
					}, $input);
				}
 ...
 */';
		...
		if($mix)
		{
			...
			if(!self::is_localhost()) self::get_build($local."/".$type, true);
	...
	public static function get_template($type)
    {
		...
		else
		{
			if($mix)
			{
				...
				{
					$file_build = self::get_build($local."/".$type, false);

					if($type == 'css')
					{
						echo '<link href="'.$url.'/css/'.$final.'?build='.$file_build.'" rel="stylesheet">'."\n";
					}
					else
					{
						echo '<script src="'.$url.'/js/'.$final.'?build='.$file_build.'"></script>'."\n";
					}
				}
			}
			else
			{
				...
				else
				{
					...
					foreach($data['file'] as $file)
					{
						...
						if($type == 'css')
						{
							echo '<link href="'.$url.'/'.($minify ? $file_min : $file_dist).'?build='.$file_build.'" rel="stylesheet">'."\n".($file === end($data['file']) ? '' : "\t");
						}
						else
						{
							echo '<script src="'.$url.'/'.($minify ? $file_min : $file_dist).'?build='.$file_build.'"></script>'."\n";
						}
					}
				}
			}
	...
	public static function get_build($path, $refresh)
	{
		if(!file_exists($path."/last-build.txt") || $refresh) {
			file_put_contents($path."/last-build.txt", date('dmYHis'));
			return date('dmYHis');
		} else {
			return file_get_contents($path."/last-build.txt");
		}
	}
    ...
    public static function http_build_url($url, $parts = array(), $flags = self::HTTP_URL_REPLACE, &$new_url = array())
    {
        ...
        if (!is_array($parts)) $parts = parse_url($parts);

        if (!isset($url['query']) && !is_string($url['query'])) $url['query'] = null;
        if (!isset($parts['query']) && !is_string($parts['query'])) $parts['query'] = null;

        $keys = array('user', 'pass', 'port', 'path', 'query', 'fragment');

        // HTTP_URL_STRIP_ALL and HTTP_URL_STRIP_AUTH cover several other flags.
        if ($flags & self::HTTP_URL_STRIP_ALL) {
            $flags |= self::HTTP_URL_STRIP_USER | self::HTTP_URL_STRIP_PASS | self::HTTP_URL_STRIP_PORT | self::HTTP_URL_STRIP_PATH | self::HTTP_URL_STRIP_QUERY | self::HTTP_URL_STRIP_FRAGMENT;
